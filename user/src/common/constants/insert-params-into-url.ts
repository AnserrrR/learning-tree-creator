 ...
 */
export const insertParamsIntoUrl = (template: string, params: Record<string, string | number>): string => {
  const urlWithInsertedParams = Object.keys(params)
    .reduce(
      (url, key) => url.replace(`:${key}`, params[key].toString()),
      template,
    );
  if (urlWithInsertedParams.includes(':')) {
    throw new InternalServerErrorException('Not all params were inserted into url');
  }
